...
TIMESTAMP_TEMPL_FILE = '/home/poxyu/work/metallurg/ex1/algorithm/timestamp_templates.xml'
...
def parse_timestamp(roi, borders, timestamp_format):
    digits = []
    for left, right in borders:
        digit = roi[:, left:right].ravel()
        digits.append(digit)
    digits = np.stack(digits)
    _, results = TIMESTAMP_MODEL.predict(digits)
...
test_time = parse_timestamp(time, TIME_DIGITS, TIME_FORMAT)
test_date = parse_timestamp(date, DATE_DIGITS, DATE_FORMAT)
